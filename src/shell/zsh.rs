...
const BEGIN_MARKER: &str = "# >>> hilia zsh hook >>>";
const END_MARKER: &str = "# <<< hilia zsh hook <<<";

pub fn hook() -> &'static str {
    r#"# 拦截「命令未找到」：明显是命令/拼写错误 → 交给系统报错；自然语言 → 交给希尔娅。
command_not_found_handler() {
    ...
    # 明显是命令（含路径/内置命令/环境变量前缀）→ 系统报错，不打扰希尔娅
    hilia --shell-classify --shell zsh -- "$@" 2>/dev/null && return 127

    hilia --shell-intercept --shell zsh -- "$@" 2>/dev/null
    return 127
}

# ---- 多行自然语言：Enter 时整块交给希尔娅（与 fish 的拦截策略一致） ----
__hilia_pending_buffer=""

# 缓冲内是否含「未知命令」行：有则视为自然语言块，交给希尔娅
__hilia_zsh_multiline_has_unknown() {
...
__hilia_zsh_accept_line() {
    local buffer="$BUFFER"
    local trimmed="${buffer#"${buffer%%[![:space:]]*}"}"
    if [[ -n "$trimmed" && "$buffer" == *$'\n'* ]]; then
        if __hilia_zsh_multiline_has_unknown "$buffer"; then
            # 整块交给希尔娅：隐藏光标、清空缓冲，下一提示符发送
            __hilia_pending_buffer="$buffer"
            print -s -- "$buffer"
            print -n '\e[?25l'
            BUFFER=""
            zle accept-line
        ...
        # （按路径查找直接报错），这里提前交给希尔娅；纯 URL 命令如 curl 不受影响
        # （它们是已知命令，不会走到这里；这里只接 URL 开头的未知命令）
        print -s -- "$buffer"
        print -rn -- '\e[?25l'
        print -rn -- $'\e[36m'"${buffer}"$'\e[0m\n'
        printf '%s' "$buffer" | hilia --shell-intercept --shell zsh --stdin 2>/dev/null
        print -rn -- '\e[?25h'
        BUFFER=""
        zle accept-line
        return
...
zle -N __hilia_zsh_accept_line
bindkey -M emacs '^M' __hilia_zsh_accept_line
bindkey -M viins '^M' __hilia_zsh_accept_line
bindkey -M vicmd '^M' __hilia_zsh_accept_line

__hilia_zsh_precmd() {
    if [[ -n "$__hilia_pending_buffer" ]]; then
        local buffer="$__hilia_pending_buffer"
        __hilia_pending_buffer=""
        print -rn -- '\e[?25h'
...
add-zsh-hook precmd __hilia_zsh_precmd
"#
}
...
mod tests {
    ...
    fn zsh_hook_wires_classifier_and_multiline_intercept() {
        ...
        assert!(hook.contains("hilia_zsh_accept_line"));
    ...
    fn zsh_hook_does_not_filter_natural_language_symbols() {
        ...
        assert!(!hook.contains("hilia_shell_syntax_pattern"));
        assert!(!hook.contains("hilia_leading_pattern"));
    }
