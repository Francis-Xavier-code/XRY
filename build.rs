...
const PROMPT_MASK: &[u8] = b"HiliaPromptMask";

fn main() {
    println!("cargo:rerun-if-changed=src/prompts/hilia.md");
    ...
    let prompt = fs::read("src/prompts/hilia.md").expect("read src/prompts/hilia.md");
    let encoded = prompt
        .into_iter()
        .enumerate()
    ...
    let dest = Path::new(&out_dir).join("default_hilia_prompt.rs");
    fs::write(
        dest,
        format!(
